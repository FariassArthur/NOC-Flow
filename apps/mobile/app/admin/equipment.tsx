...
export default function EquipmentScreen() {
  ...
  const [form, setForm] = useState({ name: '', type: 'roteador', ip: '', brand: '', equipmentModel: '', location: '', department: '', status: 'ativo' });
    ...
    <ScrollView style={{ flex: 1, backgroundColor: '#0f172a' }}>
      <View style={{ padding: 16, gap: 16 }}>
        ...
        <View style={{ backgroundColor: '#1e293b', borderRadius: 16, padding: 16, borderWidth: 1, borderColor: '#334155', gap: 12 }}>
          <Text style={{ color: '#f1f5f9', fontWeight: '600', fontSize: 15 }}>{editing ? 'Editar' : 'Novo'} Equipamento</Text>
          ...
          <View style={{ flexDirection: 'row', gap: 8 }}>
            ...
            {editing && (
              <TouchableOpacity
                onPress={() => { setEditing(null); setForm({ name: '', type: 'roteador', ip: '', brand: '', equipmentModel: '', location: '', department: '', status: 'ativo' }); }}
            ...
            <View key={item._id} style={{ backgroundColor: '#1e293b', borderRadius: 12, padding: 12, borderWidth: 1, borderColor: '#334155' }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={{ color: '#64748b', fontSize: 11, marginTop: 2 }}>{item.brand} {item.equipmentModel}</Text>
                </View>
                <View style={{ backgroundColor: item.status === 'ativo' ? '#10b981' : item.status === 'inativo' ? '#ef4444' : '#f59e0b', borderRadius: 4, paddingHorizontal: 6, paddingVertical: 2 }}>
                  <Text style={{ color: '#fff', fontSize: 10, fontWeight: '600' }}>{item.status}</Text>
                </View>
              </View>
              <View style={{ flexDirection: 'row', gap: 8, marginBottom: 8 }}>
                {item.ip && <Text style={{ color: '#94a3b8', fontSize: 10, backgroundColor: '#0f172a', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 }}>IP: {item.ip}</Text>}
                {item.location && <Text style={{ color: '#94a3b8', fontSize: 10, backgroundColor: '#0f172a', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 }}>📍 {item.location}</Text>}
              </View>
              <View style={{ flexDirection: 'row', gap: 4 }}>
