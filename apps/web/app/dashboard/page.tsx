...
export default function DashboardPage() {
  ...
  const exportCSV = () => {
    const headers = ['Título', 'Status', 'Prioridade', 'Criado por', 'Departamento', 'Criado em', 'Resolvido em', 'Tempo (min)'];
    const rows = filtered.map((o) => {
      const c = o.createdBy as any;
      return [
        `"${o.title}"`, o.status, o.priority, `"${c?.fullName || ''}"`, `"${c?.department || ''}"`,
        new Date(o.createdAt).toISOString(), o.resolvidoEm ? new Date(o.resolvidoEm).toISOString() : '',
        o.timeSpentMinutes,
      ].join(',');
    });
    const blob = new Blob([headers.join(','), '\n', rows.join('\n')], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = `ocorrencias-${new Date().toISOString().slice(0, 10)}.csv`;
    ...
    <div className="space-y-8">
      ...
      <div className="flex gap-4 items-end">
        <div>
          ...
          <input type="date" value={dateRange.from} onChange={(e) => setDateRange({ ...dateRange, from: e.target.value })} className="input-field text-sm" />
        </div>
        <div>
          <label className="block text-xs text-slate-400 mb-1">Até</label>
          <input type="date" value={dateRange.to} onChange={(e) => setDateRange({ ...dateRange, to: e.target.value })} className="input-field text-sm" />
        </div>
        {(dateRange.from || dateRange.to) && (
          <button onClick={() => setDateRange({ from: '', to: '' })} className="text-xs text-accent-500 hover:text-accent-400 mb-1">Limpar filtro</button>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="card-glow">
          <div className="flex items-center justify-between mb-4 relative z-10">
            ...
            <Link href="/dashboard/occurrences" className="text-sm text-accent-500 hover:text-accent-400">Ver todas</Link>
          </div>
          {loading ? (
            ...
            <div className="text-center py-8 relative z-10">
              ...
              <Link href="/dashboard/occurrences/new" className="btn-primary inline-block">Criar Primeira Ocorrência</Link>
            </div>
          ) : (
            <div className="space-y-3 relative z-10">
              {recent.map((occ) => {
                ...
                return (
                  <Link key={occ._id?.toString()} href={`/dashboard/occurrences/${occ._id}`}
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-slate-500 mt-1">{created?.fullName} · {created?.department}</p>
                      </div>
                      <span className={`badge-status shrink-0 ${statusConfig[occ.status]?.color || ''}`}>{statusConfig[occ.status]?.label || occ.status}</span>
                    </div>
                  </Link>
                );
              })}
