...
import { serviceAPI } from '@noc/api-client';
import type { Service } from '@noc/shared';
...
export default function ServicesPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await serviceAPI.delete(id); load(); } catch {}
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-slate-400 text-sm">Gerencie serviços de rede (Internet, MPLS, VoIP, VPN, etc.)</p>
      ...
      <form onSubmit={handleSubmit} className="card space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input type="text" value={form.contract} onChange={(e) => setForm({ ...form, contract: e.target.value })} className="input-field" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">Largura de Banda</label>
            <input type="text" value={form.bandwidth} onChange={(e) => setForm({ ...form, bandwidth: e.target.value })} className="input-field" placeholder="Ex: 500 Mbps" />
          </div>
        </div>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item._id as string} className="border-b border-slate-700/30 hover:bg-slate-700/20">
                ...
                <td className="py-3 px-4 text-slate-400 hidden md:table-cell">{item.provider || '-'}</td>
                <td className="py-3 px-4 text-slate-400 hidden md:table-cell">{item.bandwidth || '-'}</td>
                <td className="py-3 px-4 text-center hidden sm:table-cell">
                  <span className={`px-2 py-0.5 rounded-full text-xs ${item.status === 'ativo' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>
                    {item.status}
                  </span>
                </td>
            ...
            {items.length === 0 && (
              <tr><td colSpan={6} className="py-8 text-center text-slate-500">Nenhum serviço cadastrado</td></tr>
            )}
          </tbody>
        </table>
      </div>
