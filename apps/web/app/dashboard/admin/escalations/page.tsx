...
import { escalationAPI } from '@ccore/api-client';
import type { EscalationRule } from '@ccore/shared';
...
export default function EscalationsPage() {
  ...
  const [form, setForm] = useState({
    name: '',
    priority: 'alta',
    triggerType: 'sla_breach' as string,
    triggerMinutes: '60',
    targetRole: '',
    targetDepartment: '',
    notifyAlso: '',
  ...
  const handleToggle = async (item: EscalationRule) => {
    try {
      await escalationAPI.update(item._id!, { active: !item.active });
      load();
    } catch {
      /* noop */
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-slate-400 text-sm">
        Configure regras para escalonamento automático de ocorrências
      </p>
      ...
      <form onSubmit={handleSubmit} className="card space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1.5">
              Minutos para Gatilho
            </label>
            <input
              type="number"
              value={form.triggerMinutes}
              onChange={(e) => setForm({ ...form, triggerMinutes: e.target.value })}
              className="input-field"
              min={1}
              required
            />
          ...
          <div className="sm:col-span-2">
            <label className="block text-sm font-medium text-slate-300 mb-1.5">
              Notificar também (emails)
            </label>
            <input
              type="text"
              value={form.notifyAlso}
              onChange={(e) => setForm({ ...form, notifyAlso: e.target.value })}
              className="input-field"
              placeholder="Ex: admin@email.com, supervisor@email.com"
            />
          </div>
        </div>
            ...
            {items.map((item) => (
              ...
              >
                ...
                <td className="py-3 px-4 text-slate-300 hidden sm:table-cell">
                  {item.triggerType === 'sla_breach' ? 'SLA' : 'Tempo'}
                </td>
                ...
                <td className="py-3 px-4 text-slate-400 hidden lg:table-cell">
                  {item.targetRole || item.targetDepartment || '-'}
                </td>
                <td className="py-3 px-4 text-center">
                  <button
                    onClick={() => handleToggle(item)}
                    className={`px-2 py-0.5 rounded-full text-xs font-medium ${item.active ? 'bg-emerald-500/10 text-emerald-400' : 'bg-slate-600/30 text-slate-500'}`}
                  >
                    {item.active ? 'Sim' : 'Não'}
                  </button>
                </td>
