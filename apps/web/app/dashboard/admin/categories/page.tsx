...
import { categoryAPI } from '@ccore/api-client';
import type { Category } from '@ccore/shared';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = {
        ...
        slaResolutionMinutes: form.slaResolutionMinutes
          ? Number(form.slaResolutionMinutes)
          : undefined,
        color: form.color,
      };
      if (editing) {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              className="h-10 w-full rounded-lg bg-slate-800 border border-slate-600 cursor-pointer"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setForm({ ...form, slaResponseMinutes: e.target.value })}
              className="input-field"
              min={1}
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => setForm({ ...form, slaResolutionMinutes: e.target.value })}
              className="input-field"
              min={1}
            />
          </div>
            ...
            {items.map((item) => (
              ...
              >
                ...
                <td className="py-3 px-4 text-center text-slate-300 hidden md:table-cell">
                  {item.slaResponseMinutes ? `${item.slaResponseMinutes} min` : '-'}
                </td>
                <td className="py-3 px-4 text-center text-slate-300 hidden md:table-cell">
                  {item.slaResolutionMinutes ? `${item.slaResolutionMinutes} min` : '-'}
                </td>
