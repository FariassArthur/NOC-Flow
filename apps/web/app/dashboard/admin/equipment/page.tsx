  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-slate-400 text-sm">
        Gerencie equipamentos de rede associados às ocorrências
      </p>
            ...
            {items.map((item) => (
              ...
              >
                ...
                <td className="py-3 px-4 text-center hidden sm:table-cell">
                  <span
                    className={`px-2 py-0.5 rounded-full text-xs ${item.status === 'ativo' ? 'bg-emerald-500/10 text-emerald-400' : item.status === 'manutencao' ? 'bg-amber-500/10 text-amber-400' : 'bg-red-500/10 text-red-400'}`}
                  >
                    {item.status}
                  </span>
                </td>
