...
import { runbookAPI, categoryAPI } from '@ccore/api-client';
import type { Runbook, Category } from '@ccore/shared';
...
export default function RunbooksPage() {
  ...
  const [form, setForm] = useState({
    ...
    priority: '',
    tags: '',
    steps: [{ order: 1, description: '' }] as { order: number; description: string }[],
  });
  ...
  const addStep = () =>
    setForm({ ...form, steps: [...form.steps, { order: form.steps.length + 1, description: '' }] });
  ...
  const updateStep = (idx: number, val: string) =>
    setForm({
      ...form,
      steps: form.steps.map((s, i) => (i === idx ? { ...s, description: val } : s)),
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-slate-400 text-sm">
        Procedimentos operacionais padronizados para resposta a incidentes
      </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input
              ...
              className="input-field"
              placeholder="Ex: bgp, link, provedor"
            />
          </div>
        </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {item.priority && (
                        <span
                          className={`px-2 py-0.5 rounded-full text-xs ${item.priority === 'crítica' ? 'bg-red-500/10 text-red-400' : 'bg-slate-600/30 text-slate-400'}`}
                        >
                          {item.priority}
                        </span>
                      )}
                      ...
                      <div className="mt-4 space-y-2 border-t border-slate-700/30 pt-4">
                        {item.steps.map((step) => (
                          <div key={step.order} className="flex gap-3 text-sm">
                            <span className="w-6 h-6 rounded-full bg-accent-500/20 text-accent-500 flex items-center justify-center text-xs font-bold shrink-0">
                              {step.order}
                            </span>
